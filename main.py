...
async def fight(ctx):
    ...
    if voice_channel:
        ...
        if len(voice_members) > 1:
            f1, f2 = [voice_members.pop(random.randint(0, len(voice_members))), voice_members.pop(random.randint(0, len(voice_members)-1))]
        else:
            f1 = ctx.author
        ...
        await ctx.channel.send("ээээ второго нет")
